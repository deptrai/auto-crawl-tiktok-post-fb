...
- (Story 2.3, forward Epic 4) Khi tạo bảng `automation_jobs`, khai báo FK `profile_id ... REFERENCES profiles(id) ON DELETE CASCADE` để `phase3:profile:delete` (2.3) tự động dọn jobs liên quan khi xóa profile. Nếu KHÔNG dùng CASCADE → phải bổ sung xóa automation_jobs vào `ProfileService.deleteProfile`. AC epic 2.3 gốc yêu cầu "automation_jobs liên quan đều bị xóa" — hiện defer vì bảng chưa tồn tại. [automation-desktop/src/main/profile/profile-service.ts deleteProfile]

- (Story 3.2, forward Epic 6) Wire telemetry beacon `proxy_error` vào hook `onProxyError` của ProxyService (gọi khi circuit breaker mở). Hiện defer vì telemetry/index.ts chỉ là placeholder (export {}). Khi Epic 6 build beacon-emitter → connect hook này. [automation-desktop/src/main/proxy/proxy-service.ts onProxyError]

## Deferred from: code review of story-4.6b (2026-06-03)

- Không có poll-timeout/max-poll → renderer poll `automation:status` vô hạn nếu job kẹt non-terminal (mạng hang không throw, không đạt timeout Playwright). Cần thiết kế UX timeout + đánh dấu job stuck → FAILED sau N giây. [automation-desktop/src/renderer/src/views/ProfilesView.tsx:263-294]
- `automationError` là state global (1 string), không attribution per-profile → lỗi automation của profile A bị ghi đè bởi profile B; start B xóa lỗi A. Nên đổi thành `Record<profileId, string>` hoặc hiển thị lỗi trong row từng profile. [automation-desktop/src/renderer/src/views/ProfilesView.tsx:70,450]
- `normalizeError` nuốt error (`void error`) ở cả 2 handler — không log non-secret để debug production. Nên log qua `redact()` (rule #11) khi có logger abstraction. [automation-desktop/src/main/ipc/automation-handlers.ts:29; content-template-handlers.ts:32]
- `extractTokens` dùng `page.content?.() ?? ''` → trả empty HTML âm thầm nếu page không hỗ trợ `content()`, token extractor fail im lặng thay vì throw rõ. Real Playwright luôn có `content()` nên risk thấp; nên throw explicit khi `!page.content`. [automation-desktop/src/main/adapters/electron-bootstrap.ts:245]
- Zombie jobs non-terminal tồn đọng trong `automation_jobs` sau khi app bị force-close (job ở WARMING_UP...). `automation-job-repo.listResumable` có sẵn nhưng không được dùng trong `bootstrapApplication` để force-FAILED orphan jobs. Job-resume là phạm vi rộng hơn (Epic-level). [automation-desktop/src/main/adapters/electron-bootstrap.ts]
- Thiếu unit test orchestrator `runSelfComment` với `options.target` tùy chỉnh (tất cả test hiện dùng default `/me`). Hành vi custom target chỉ được cover gián tiếp qua integration test automation-handlers. [automation-desktop/tests/unit/self-comment-orchestrator.spec.ts]

## Deferred from: code review of story-12.1 (2026-06-04)
