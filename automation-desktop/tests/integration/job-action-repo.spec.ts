...
function buildFixture(): { dir: string; entry: string; resultPath: string } {
  const dir = mkdtempSync(join(tmpdir(), 'phase3-job-action-repo-fixture-'))
  const entry = join(dir, 'job-action-repo-electron-entry.cjs')
  const resultPath = join(dir, 'result.json')
  buildSync({
    entryPoints: [join(process.cwd(), 'tests/fixtures/job-action-repo-electron-entry.ts')],
    outfile: entry,
    bundle: true,
    platform: 'node',
    format: 'cjs',
...
test('[P0] job action repository records jti reference and cascades with automation job in real SQLCipher', async () => {
  ...
  try {
    await execFileAsync(join(process.cwd(), 'node_modules/.bin/electron'), [fixture.entry], {
      env: {
        ...
        JOB_ACTION_SMOKE_RESULT_PATH: fixture.resultPath
      },
      timeout: 30_000
    })
    ...
    expect(result).toEqual({
      ok: true,
      checks: ['record-jti-only', 'fk-enforced', 'cascade-delete']
    })
  } finally {
