export { registerSettingsHandlers, type IpcMainLike } from './settings-handlers'
export { registerShellHandlers, type OpenExternal } from './shell-handlers'
export { registerLicenseHandlers } from './license-handlers'
export { registerProfileHandlers } from './profile-handlers'
export { registerProxyHandlers } from './proxy-handlers'
export { registerCaptchaHandlers } from './captcha-handlers'
export { registerContentTemplateHandlers } from './content-template-handlers'
export { registerTargetListHandlers } from './target-list-handlers'
export { registerAutomationHandlers } from './automation-handlers'
export { registerMessengerHandlers } from './messenger-handlers'
